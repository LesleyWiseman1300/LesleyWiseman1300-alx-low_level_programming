...
Like, comment, subscribe mandatory Copy this main file. Comment out (dot!) the part of the code that is causing the output to go into an infinite loop.
DonÎndd or remove any lines of code, as we will be checking your line count. You are only allowed to comment out existing code. You do not have to compile with -Wall -Werror -Wextra -pedantic for this task.
